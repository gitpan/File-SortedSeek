...
my $default_rec_sep = ($^O =~ m/win32/i) ? "\015\012" :
                      ( $^O =~ /mac/i )  ? "\015"     : "\012";
...
sub _alphabetic_compare {
    my ($x, $y) = @_;
    return $descending ? $y cmp $x : $x cmp $y;
}
...
sub alphabetic {
    ...
    $error_msg   = '';
    _look( *FILE, $string, { xfrm => $munge_ref,
                             comp => \&_alphabetic_compare,
                             dict => $dict_order,
                             fold => $ignore_case } );
}
...
sub numeric {
    ...
    my $munge_ref   = shift;
    $error_msg   = '';
    _look( *FILE, $number, { xfrm => $munge_ref, comp => \&_numeric_compare } );
}
...
sub find_time {
    ...
    _look( *FILE, $find, { xfrm => $munge_ref, comp => \&_numeric_compare } );
}
...
sub get_epoch_seconds {
  ...
  return undef unless defined $line;
    ...
    my ($wday,$mon,$mday,$hours,$min,$sec,$year) =
        $line =~ m/(\w{3})\s+(\w{3})\s+(\d{1,2})\s+(\d\d):(\d\d):(\d\d)\s+(\d{4})/;
    unless ($year) {
...
sub get_last {
    ...
    my($file_size, $blksize) = @stat[7,11];
    $blksize ||= 8192;
    # grab the first chunk from eof at block offset
    my $pos = $file_size - (($file_size % $blksize)|| $blksize );
    my $file = '';
    my ($buf, $lines);
    for(;;) {
        $pos = 0 if $pos < 0;
        seek(FILE,$pos,0);
        read(FILE, $buf, $blksize);
        $file = $buf.$file;
...
# Search::Dict not used directly as it does not support cuddle functionality
...
file lines so that suitable values are extracted for comparison to $find.
    ...
    $tell = alphabetic( *BIG, $find, \&munge_line, $dict_order, $ignore_case );
...
The alphabetic function also takes optional 4th and 5th arguments.

If I<$dict_order> is true, search by dictionary order (ignore anything but word
characters and whitespace).  The default is honour all characters.

If I<$ignore_case> is true, ignore case.  The default is to honour case.
...
The binmoding of the FH by get_between() and get_last() can not be
easily avoided.

=head2 CREDITS

Peter (Stig) Edwards for bugfixes and refactoring.
...
(c) Dr James Freeman 2000-01 E<lt>airmedical@gmail.comE<gt>
All rights reserved.
